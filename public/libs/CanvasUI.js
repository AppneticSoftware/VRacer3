// From: https://github.com/NikLever/CanvasUI
import {
  Mesh,
  CanvasTexture,
  MeshBasicMaterial,
...
class CanvasUI {
        ...
        if (elm.type == "button") {
          ...
          if (self.keyboard) {
            if (self.keyboard.visible) {
              ...
              const y =
                (0.5 -
                  (elm.position.y + elm.height + self.config.body.padding) /
                    self.config.height) *
                self.panelSize.height;
              const h =
                Math.max(self.panelSize.width, self.panelSize.height) / 2;
  ...
  update() {
    ...
    Object.entries(this.content).forEach(([name, content]) => {
      const config =
        self.config[name] !== undefined ? self.config[name] : self.config.body;
        ...
        if (
          ...
          if (hover) {
            if (!svgPath && config.type == "button") {
              context.fillStyle =
                config.fontColor !== undefined ? config.fontColor : fontColor;
            } else {
              context.fillStyle =
  ...
  set position(value) {
    ...
    if (!(value instanceof Vector3)) {
      console.error(
        "CanvasUI trying to set the mesh position using a parameter that is not a THREE.Vector3"
      );
      return;
    }
  ...
  set quaternion(value) {
    ...
    if (!(value instanceof QUaternion)) {
      console.error(
        "CanvasUI trying to set the mesh quaternion using a parameter that is not a THREE.Quaternion"
      );
      return;
    }
